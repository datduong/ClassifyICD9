...
def all_metrics(yhat, y, k=1, yhat_raw=None, calc_auc=True,input_label_energy=None):
  """
    Inputs:
  ...
  if yhat_raw is not None and calc_auc:
    ...
    for k_i in k:
      ...
      if input_label_energy is None: 
        rec_at_k = recall_at_k(yhat_raw, y, k_i)
        prec_at_k = precision_at_k(yhat_raw, y, k_i)
      else: 
        rec_at_k = recall_at_k_break_tie(yhat_raw, y, k_i, input_label_energy)
        prec_at_k = precision_at_k_break_tie ( yhat_raw, y, k_i, input_label_energy )
...
def recall_at_k_break_tie(yhat_raw, y, k, input_label_energy): # @input_label_energy is used to break tie
  # m = zip([0,0,2,1],[1,0,2,0])
  # sorted(m,key=lambda x: (-x[0],-x[1])) ## sort by 0 first array, then break tie by 2nd array. the negative is sorted from largest to smallest 

  # get precision at k for each example, and do tie break 
  vals = []
  location = range(yhat_raw.shape[1]) ## need this to backtrack which labels 
  for i in range(yhat_raw.shape[0]): ## each person 
    array_yhat_energy = zip ( yhat_raw[i], input_label_energy[i], location )
    sortd = sorted(array_yhat_energy,key=lambda x: (-x[0],-x[1])) ## sort by 0 first array, then break tie by 2nd array. 
    sortd = sortd[0:k] ## top y
    tk = [k[2] for k in sortd] ## take the indexing only
    num_true_in_top_k = y[i,tk].sum() ## check if these top indexes are true labels 
    denom = y[i,:].sum()
    vals.append(num_true_in_top_k / float(denom))
...
def precision_at_k_break_tie(yhat_raw, y, k, input_label_energy): # @input_label_energy is used to break tie
  # m = zip([0,0,2,1],[1,0,2,0])
  # sorted(m,key=lambda x: (-x[0],-x[1])) ## sort by 0 first array, then break tie by 2nd array. the negative is sorted from largest to smallest 
